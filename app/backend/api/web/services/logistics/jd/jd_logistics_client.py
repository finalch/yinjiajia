...
from .jd_order_model import JdOrderRequest, JdSenderInfo, JdReceiverInfo, JdCargoDetail, JdOrderResponse
...
class JdLogisticsClient(BaseLogisticsClient):
    ...
    def query_order(self, tracking_number: str) -> LogisticsResponse:
        ...
        try:
            ...
            # 调用京东API查询
            response = self.client.query_order(waybill_no)
    ...
    def _parse_create_response(self, response: Dict[str, Any], tracking_number: str) -> LogisticsResponse:
        ...
        try:
            if response.get('success', False):
                ...
                waybill_no = result_data.get('waybillNo')
    ...
    def _parse_query_response(self, response: Dict[str, Any], tracking_number: str) -> LogisticsResponse:
        """解析查询订单响应"""
        try:
            if response.get('success', False):
                result_data = response.get('result', {})
                ...
                error_msg = response.get('error', '查询失败')
                return LogisticsResponse(
                    success=False,
            ...
            return LogisticsResponse(
                success=False,
                error_message=f"解析查询响应失败: {str(e)}"
            )
