...
class WebSocketHandler:
    ...
    def start_server(self, host='0.0.0.0', port=8003):
        """启动WebSocket服务器"""
    ...
    async def handle_connection(self, websocket: WebSocketServerProtocol):
        ...
        try:
            # 调试信息：打印websocket对象的属性
            # logger.info(f"WebSocket对象属性: {dir(websocket)}")
            # logger.info(f"WebSocket request_headers: {getattr(websocket, 'request_headers', 'None')}")
            request = getattr(websocket, 'request')
            # 从请求头获取查询参数
            # 在websockets库中，查询参数通常通过request_headers传递
            query_string = hasattr(request, 'path')
            # 解析查询参数
            auth_params = {}
