...
export const cartApi = {
	// 获取购物车列表
	getCart(userId = 1) {
		return request.get('/api/app/cart/', { user_id: userId })
	},
...
export const orderApi = {
	...
	getOrderDetail(orderId, userId = 1) {
		return request.get(`/api/app/order/${orderId}`, { user_id: userId })
	},
	...
	cancelOrder(orderId, userId = 1) {
		return request.post(`/api/app/order/${orderId}/cancel`, { user_id: userId })
	}
}
...
export const authApi = {
  register({ phone, password }) {
    return request.post('/api/app/auth/register', { phone, password })
  },
  login({ phone, password }) {
    return request.post('/api/app/auth/login', { phone, password })
  },
  validate(token) {
    return request.get('/api/app/auth/validate', { token })
  }
}
