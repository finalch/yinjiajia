<template>
  <view class="detail-container">
    ...
    <!-- 订单信息 -->
    <view class="card">
      <view class="row"><text class="label">订单号</text><text class="value">{{ order.order_number }}</text></view>
      <view class="row"><text class="label">下单时间</text><text class="value">{{ formatDate(order.created_at) }}</text></view>
      <view class="row"><text class="label">订单状态</text><text class="value status" :class="statusClass">{{ order.status_text }}</text></view>
      <view class="row total"><text class="label">合计</text><text class="value price">¥{{ formatPrice(order.total_amount) }}</text></view>
    </view>

    <!-- 物流信息 -->
    <view class="card" v-if="order.logistics">
      <view class="card-title">物流信息</view>
      <view class="row"><text class="label">物流公司</text><text class="value">{{ order.logistics.carrier }}</text></view>
      <view class="row"><text class="label">运单号</text><text class="value">{{ order.logistics.tracking_number }}</text></view>
      <view class="row"><text class="label">状态</text><text class="value">{{ order.logistics.status_text || order.logistics.status }}</text></view>
      <view class="row"><text class="label">更新时间</text><text class="value">{{ formatDate(order.logistics.updated_at) }}</text></view>
    </view>
    ...
    <view class="card">
      ...
      <view class="item" v-for="it in order.items" :key="it.id">
        <view class="thumb"><image :src="it.product_image || '/static/default-product.png'" /></view>
        <view class="info">
          <view class="name" :title="it.product_name">{{ it.product_name }}</view>
          <view class="spec" v-if="it.spec_combination_id">规格：{{ it.spec_combination_id }}</view>
          <view class="meta">
