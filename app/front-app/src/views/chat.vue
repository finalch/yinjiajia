    ...
    <div class="messages-container" ref="messagesContainer">
      ...
      <div v-else class="messages-list">
        <div
          ...
          :key="message.id"
          :class="['message-item', message.sender_type === 'user' ? 'user-message' : 'merchant-message']"
        >
  ...
  methods: {
    async initChat() {
      try {
        ...
        if (!this.userId || !this.merchantId) {
          this.$toast('参数错误')
          this.goBack()
          return
        }
        ...
        // 用户加入聊天
        websocketService.joinAsUser(this.userId)
    ...
    setupEventListeners() {
      ...
      websocketService.on('online_status', (status) => {
        this.isOnline = status.merchant_online
      })

      // 监听错误
      websocketService.on('error', (error) => {
        console.error('WebSocket错误:', error)
        this.$toast(error.message || '发生错误')
      })
    },
